...
export function VisualEditor({
  ...
  // Contract state
  const [contract, setContract] = useState<VisualContract>(() => {
  ...
  const [codeGenerationResult, setCodeGenerationResult] = useState<CCLGenerationResult | null>(null)
  const [isDeploying, setIsDeploying] = useState(false)

  // Update contract when it changes
  const updateContract = useCallback((updates: Partial<VisualContract>) => {
  ...
  // Node management
  const handleNodeCreate = useCallback((component: PaletteComponent, position: Position) => {
    if (readOnly) return
  ...
  const handleNodeUpdate = useCallback((nodeId: string, updates: Partial<CanvasNode>) => {
    ...
    const updatedNodes = contract.nodes.map(node =>
  ...
  const handleNodeDelete = useCallback((nodeId: string) => {
    ...
    const updatedConnections = contract.connections.filter(
  ...
  const handleConnectionCreate = useCallback((connection: Omit<Connection, 'id'>) => {
    ...
    const existingConnection = contract.connections.find(
      conn => 
  ...
  const handleDeploy = useCallback(async () => {
    if (!codeGenerationResult?.valid || !onContractDeploy) return
  ...
  return (
    <div className={`visual-editor h-full flex flex-col bg-gray-100 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between p-4 bg-white border-b border-gray-200">
        <div className="flex items-center gap-4">
          <h1 className="text-xl font-semibold text-gray-900">CCL Visual Editor</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600">
            <span>{contract.nodes.length} components</span>
            <span>{contract.connections.length} connections</span>
          </div>

          {/* Actions */}
          {codeGenerationResult?.valid && onContractDeploy && (
            <button
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 flex-shrink-0">
          <ComponentPalette
            onComponentSelect={handleNodeCreate}
      ...
      <div className="px-4 py-2 bg-gray-50 border-t border-gray-200">
        <div className="flex items-center justify-between text-sm text-gray-600">
          <div className="flex items-center gap-4">
            ...
            <span>Version: {contract.metadata.version}</span>
          </div>
          <div className="flex items-center gap-4">
            ...
            <span>Last modified: {contract.metadata.modified.toLocaleTimeString()}</span>
          </div>
        </div>
      </div>
    </div>
