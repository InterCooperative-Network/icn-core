...
impl Optimizer {
    ...
    fn fold_ast(&mut self, ast: AstNode) -> AstNode {
        match ast {
            AstNode::Program(nodes) => {
                let optimized_nodes = nodes.into_iter()
                    .map(|node| match node {
            ...
            AstNode::Policy(stmts) => {
                let optimized_stmts = stmts.into_iter()
            ...
            AstNode::ContractDeclaration { name, metadata, body } => {
                AstNode::ContractDeclaration {
                    name,
                    metadata,
                    body: body.into_iter().map(|item| self.fold_contract_body(item)).collect(),
                }
            }
    ...
    fn fold_contract(&mut self, contract: ContractDeclarationNode) -> ContractDeclarationNode {
        ContractDeclarationNode {
            ...
            body: contract.body.into_iter()
                .map(|item| self.fold_contract_body(item))
                .collect(),
        }
    }

    fn fold_contract_body(&mut self, body: crate::ast::ContractBodyNode) -> crate::ast::ContractBodyNode {
        use crate::ast::ContractBodyNode;
        match body {
            ContractBodyNode::Role(role) => ContractBodyNode::Role(self.fold_role(role)),
            ContractBodyNode::Proposal(proposal) => ContractBodyNode::Proposal(self.fold_proposal(proposal)),
            ContractBodyNode::Function(func) => ContractBodyNode::Function(self.fold_function_definition(func)),
            ContractBodyNode::State(state) => ContractBodyNode::State(self.fold_state_declaration(state)),
            ContractBodyNode::Struct(struct_def) => ContractBodyNode::Struct(self.fold_struct_definition(struct_def)),
            ContractBodyNode::Enum(enum_def) => ContractBodyNode::Enum(self.fold_enum_definition(enum_def)),
            ContractBodyNode::Const(const_def) => ContractBodyNode::Const(self.fold_const_declaration(const_def)),
        }
    }
    ...
    fn fold_proposal(&mut self, proposal: ProposalDeclarationNode) -> ProposalDeclarationNode {
        ...
        ProposalDeclarationNode {
            name: proposal.name,
            fields: proposal.fields.into_iter().map(|field| {
                match field {
    ...
    fn fold_type_expr(&mut self, type_expr: TypeExprNode) -> TypeExprNode {
        match type_expr {
            TypeExprNode::Array(inner) => TypeExprNode::Array(Box::new(self.fold_type_expr(*inner))),
            TypeExprNode::Map { key_type, value_type } => TypeExprNode::Map {
                key_type: Box::new(self.fold_type_expr(*key_type)),
                value_type: Box::new(self.fold_type_expr(*value_type)),
            },
            TypeExprNode::Option(inner) => TypeExprNode::Option(Box::new(self.fold_type_expr(*inner))),
    ...
    fn fold_block(&mut self, block: BlockNode) -> BlockNode {
        let optimized_stmts: Vec<StatementNode> = block.statements.into_iter()
    ...
    fn fold_stmt(&mut self, stmt: StatementNode) -> StatementNode {
        match stmt {
            ...
            StatementNode::If { condition, then_block, else_ifs, else_block } => {
                ...
                } else if let ExpressionNode::Literal(LiteralNode::Boolean(false)) = folded_condition {
            ...
            StatementNode::Return(expr) => {
                StatementNode::Return(expr.map(|e| self.fold_expr(e)))
            }
            StatementNode::Emit { event_name, fields } => {
                StatementNode::Emit {
                    event_name,
    ...
    pub fn fold_expr(&mut self, expr: ExpressionNode) -> ExpressionNode {
        match expr {
            ...
            // Legacy expressions
            ExpressionNode::IntegerLiteral(val) => ExpressionNode::Literal(LiteralNode::Integer(val)),
            ExpressionNode::StringLiteral(val) => ExpressionNode::Literal(LiteralNode::String(val)),
            ExpressionNode::BooleanLiteral(val) => ExpressionNode::Literal(LiteralNode::Boolean(val)),
    ...
    fn fold_binary_op(&mut self, left: ExpressionNode, op: BinaryOperator, right: ExpressionNode) -> ExpressionNode {
    ...
    fn fold_unary_op(&mut self, op: UnaryOperator, operand: ExpressionNode) -> ExpressionNode {
        ...
        match (&op, &operand) {
            ...
            (UnaryOperator::Not, ExpressionNode::UnaryOp { 
    ...
    fn is_dead_code(&self, stmt: &StatementNode) -> bool {
        match stmt {
            StatementNode::ExpressionStatement(ExpressionNode::Literal(LiteralNode::Integer(0))) => true,
            _ => false,
        }
    }
}
...
mod tests {
    use super::*;
    use crate::ast::{LiteralNode, BinaryOperator};
