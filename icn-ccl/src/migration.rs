    ...
    pub fn parse(version_str: &str) -> Result<Self, CclError> {
        ...
        if parts.len() != 3 {
            return Err(CclError::CliArgumentError(
                format!("Invalid version format: {}. Expected x.y.z", version_str)
            ));
        }
...
/// Current CCL version
pub const CURRENT_CCL_VERSION: CclVersion = CclVersion { major: 0, minor: 2, patch: 0 };
...
impl MigrationEngine {
    ...
    pub fn detect_version(&self, content: &str) -> Result<CclVersion, CclError> {
        ...
        if let Some(version_line) = content.lines().find(|line| line.contains("// CCL Version:")) {
    ...
    pub fn migrate(&self, content: &str, from_version: &CclVersion, to_version: &CclVersion) -> Result<String, CclError> {
    ...
    fn apply_rule(&self, content: &str, rule: &MigrationRule) -> Result<String, CclError> {
        ...
        match rule.rule_type {
            MigrationRuleType::TextReplace => {
                Ok(content.replace(&rule.pattern, &rule.replacement))
            }
            MigrationRuleType::KeywordReplace => {
                // Use word boundaries to avoid partial replacements
                let pattern = format!(r"\b{}\b", regex::escape(&rule.pattern));
                let regex = Regex::new(&pattern).map_err(|e| {
                    CclError::ParsingError(format!("Invalid regex pattern: {}", e))
                })?;
                Ok(regex.replace_all(content, rule.replacement.as_str()).to_string())
            }
            MigrationRuleType::RegexReplace | MigrationRuleType::FunctionSignature | MigrationRuleType::SyntaxChange => {
    ...
    pub fn generate_migration_report(&self, content: &str, from_version: &CclVersion, to_version: &CclVersion) -> MigrationReport {
        ...
        if content.contains("unsafe") {
            report.warnings.push("Contract contains 'unsafe' code that may need manual review".to_string());
        }
...
impl MigrationReport {
    pub fn to_string(&self) -> String {
        ...
        report.push_str(&format!("Migration Report: {} -> {}\n", 
            self.from_version.to_string(), self.to_version.to_string()));
        report.push_str(&format!("Estimated changes: {}\n\n", self.estimated_changes));

        if !self.applicable_rules.is_empty() {
            report.push_str("Applicable migration rules:\n");
...
pub fn convert_from_solidity(solidity_content: &str) -> Result<String, CclError> {
    ...
    ccl_content.push_str("// Manual review and adjustment is required for correct functionality.\n");
...
/// Migrate a CCL file
pub fn migrate_file(input_path: &Path, output_path: &Path, target_version: Option<CclVersion>) -> Result<(), CclError> {
    ...
    println!("Detected version: {}", detected_version.to_string());
    println!("Target version: {}", target.to_string());
