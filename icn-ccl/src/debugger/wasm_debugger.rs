...
impl WasmDebugger {
    ...
    pub fn step_next(&mut self) -> Result<DebuggerState, String> {
        match &self.state {
            DebuggerState::Paused { location } | DebuggerState::Stepped { location } => {
                ...
                self.state = DebuggerState::Stepped { location: next_location.clone() };
            ...
            DebuggerState::Stopped => {
                Err("Cannot step when debugger is stopped".to_string())
            }
        ...
        match &self.state {
            DebuggerState::Paused { location } | DebuggerState::Stepped { location } => {
                ...
                if self.is_function_call_at_location(location) {
                    ...
                    self.state = DebuggerState::Stepped { location: entry_location };
            ...
            DebuggerState::Paused { .. } | DebuggerState::Stepped { .. } => {
                if self.call_stack.len() > 1 {
                    ...
                    if let Some(caller_frame) = self.call_stack.last() {
                        ...
                        self.state = DebuggerState::Stepped { location: return_location };
                        Ok(self.state.clone())
                    } else {
        ...
        match &self.state {
            DebuggerState::Paused { .. } | DebuggerState::Stepped { .. } => {
                ...
                if let Some(current_frame) = self.call_stack.last_mut() {
                    ...
                    current_frame.locals.insert(variable_name.to_string(), validated_value);
                    Ok(())
                } else {
                    Err("No execution context available".to_string())
                }
    ...
    fn refresh_locals_at_location(&mut self, location: &WasmLocation) -> Result<(), String> {
        if let Some(current_frame) = self.call_stack.last_mut() {
            ...
            current_frame.locals.insert("local_var".to_string(), "42".to_string());
            current_frame.locals.insert("param1".to_string(), "hello".to_string());
        }
    ...
    fn push_stack_frame(&mut self, function_index: u32) {
        ...
        let ccl_location = self.source_map.find_ccl_location(&WasmLocation {
            function_index,
            instruction_offset: 0,
        }).cloned();
    ...
    fn evaluate_arithmetic_expression(&self, expression: &str, frame: &StackFrame) -> Result<String, String> {
    ...
    fn apply_operator(&self, left: &str, op: &str, right: &str) -> Result<String, String> {
        let left_num = left.parse::<i64>().map_err(|_| "Left operand is not a number")?;
        let right_num = right.parse::<i64>().map_err(|_| "Right operand is not a number")?;
    ...
    fn evaluate_function_call(&self, expression: &str, _frame: &StackFrame) -> Result<String, String> {
