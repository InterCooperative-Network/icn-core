//! Zero-knowledge proof helpers for verifiable credentials.
//!
//! This module defines pluggable [`ZkProver`] and [`ZkVerifier`] traits and
//! includes a [`Groth16Verifier`] capable of validating proofs generated by
//! circuits in the [`icn-zk`] crate.
...
/// Verifier for Groth16 proofs using a prepared verifying key and
...
impl Groth16Verifier {
    /// Create a new verifier with the given verifying key and public inputs.
    pub fn new(
        vk: ark_groth16::PreparedVerifyingKey<ark_bn254::Bn254>,
        public_inputs: Vec<ark_bn254::Fr>,
    ) -> Self {
...
impl ZkVerifier for Groth16Verifier {
    fn verify(&self, proof: &ZkCredentialProof) -> Result<bool, ZkError> {
        ...
        Groth16::<ark_bn254::Bn254>::verify_proof(&self.vk, &groth_proof, &self.public_inputs)
            .map_err(|_| ZkError::VerificationFailed)
    }
}
...
mod tests {
    ...
    fn groth16_verifier_ok() {
        ...
        let verifier = Groth16Verifier::new(vk, inputs);
        assert!(verifier.verify(&proof).unwrap());
    }
    ...
    fn groth16_backend_mismatch() {
        ...
        let vk = prepare_vk(&pk);
        let verifier = Groth16Verifier::new(vk, vec![ark_bn254::Fr::from(2020u64)]);
        assert!(matches!(
            verifier.verify(&proof),
            Err(ZkError::UnsupportedBackend(_))
        ));
    ...
    #[test]
    fn groth16_malformed_proof_error() {
        use ark_serialize::CanonicalSerialize;
        use ark_std::rand::{rngs::StdRng, SeedableRng};
        use icn_zk::{prepare_vk, prove, setup, AgeOver18Circuit};
