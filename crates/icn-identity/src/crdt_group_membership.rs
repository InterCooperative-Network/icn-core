...
impl CRDTGroupMembership {
    ...
    pub fn create_group(&self, group_id: &str) -> Result<(), CommonError> {
        ...
        if self.group_exists(group_id) {
            return Err(CommonError::InvalidInputError(format!("Group {} already exists", group_id)));
        }
        
        let mut map = self.membership_map.write()
            .map_err(|_| CommonError::LockError("Failed to acquire write lock".to_string()))?;
    ...
    pub fn create_group_with_members(&self, group_id: &str, members: &[Did]) -> Result<(), CommonError> {
        ...
        debug!("Created group {} with {} initial members", group_id, members.len());
        Ok(())
    }
    ...
    pub fn add_member(&self, group_id: &str, member: &Did) -> Result<(), CommonError> {
        ...
        if !self.group_exists(group_id) {
            if self.config.auto_create_groups {
                ...
                return Err(CommonError::InvalidInputError(format!("Group {} does not exist", group_id)));
            }
        }
    ...
    pub fn remove_member(&self, group_id: &str, member: &Did) -> Result<(), CommonError> {
        ...
        debug!("Successfully removed member {} from group {}", member, group_id);
        Ok(())
    }
    ...
    fn get_or_create_or_set(&self, group_id: &str) -> Result<ORSet<String>, CommonError> {
        ...
        if let Some(or_set) = map.get(&group_id.to_string()) {
            ...
            map.put(group_id.to_string(), or_set.clone(), self.node_id.clone())
                .map_err(|e| CommonError::CRDTError(format!("Failed to create group OR-Set: {}", e)))?;
...
mod tests {
    ...
    fn test_crdt_group_membership_initial_groups() {
        ...
        initial_groups.insert("admins".to_string(), vec![
            "did:key:alice".to_string(),
            "did:key:bob".to_string(),
        ]);
        initial_groups.insert("users".to_string(), vec![
            "did:key:charlie".to_string(),
        ]);
