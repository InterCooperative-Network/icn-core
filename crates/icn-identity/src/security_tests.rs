...
mod security_tests {
    ...
    fn test_did_validation_rejects_malicious_inputs() {
        ...
        for malicious_did in malicious_dids {
            ...
            assert!(result.is_err(), "Should reject malicious DID: {:?}", malicious_did);
        }
    }
    ...
    fn test_did_key_validation_rejects_invalid_formats() {
        ...
        for invalid_did in invalid_did_keys {
            ...
            assert!(result.is_err(), "Should reject invalid did:key: {:?}", invalid_did);
        }
    }
    ...
    fn test_did_peer_validation_rejects_invalid_algorithms() {
        ...
        for invalid_did in invalid_did_peers {
            let result = secure_validate_did(&invalid_did, &config);
            assert!(result.is_err(), "Should reject invalid did:peer: {:?}", invalid_did);
        }
    }
    ...
    fn test_execution_receipt_tamper_resistance() {
        ...
        tampered.cpu_ms = 200;
        assert!(tampered.verify_against_did(&did).is_err(), "Should detect cpu_ms tampering");
        
        let mut tampered = signed_receipt.clone();
        tampered.success = false;
        assert!(tampered.verify_against_did(&did).is_err(), "Should detect success tampering");
        
        // Test signature byte tampering
        let mut tampered = signed_receipt.clone();
        if !tampered.sig.0.is_empty() {
            tampered.sig.0[0] = tampered.sig.0[0].wrapping_add(1);
            assert!(tampered.verify_against_did(&did).is_err(), "Should detect signature tampering");
        }
    }
    ...
    fn test_security_audit_comprehensive() {
        ...
        let _has_critical = audit_result.issues.iter().any(|issue| {
            matches!(issue.severity, SecurityIssueSeverity::Critical)
        });
        // Note: Our current audit might not flag Critical issues, but High/Medium issues should exist
        let has_high = audit_result.issues.iter().any(|issue| {
            matches!(issue.severity, SecurityIssueSeverity::High)
        });
    ...
    fn test_constant_time_signature_verification() {
        ...
        let avg_valid: std::time::Duration = valid_times.iter().sum::<std::time::Duration>() / valid_times.len() as u32;
        let avg_invalid: std::time::Duration = invalid_times.iter().sum::<std::time::Duration>() / invalid_times.len() as u32;
    ...
    fn test_error_messages_dont_leak_sensitive_info() {
        ...
        if let Err(error) = result {
            ...
            assert!(!error_msg.contains("00000000"), "Error message contains sensitive data");
    ...
    fn test_multicodec_validation_edge_cases() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Unsupported multicodec"));
    }
    ...
    fn test_base58_validation() {
        ...
        for invalid_char in invalid_base58_chars {
            ...
            assert!(result.is_err(), "Should reject invalid base58 character: {}", invalid_char);
        }
    }
}