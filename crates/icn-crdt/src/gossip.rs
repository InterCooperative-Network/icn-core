...
pub trait GossipTransport: Send + Sync {
    ...
    async fn send_message(&self, peer: &PeerInfo, message: GossipMessage) -> CRDTResult<GossipResponse>;
    
    /// Broadcast a message to multiple peers.
    async fn broadcast_message(&self, peers: &[PeerInfo], message: GossipMessage) -> Vec<CRDTResult<GossipResponse>>;
...
impl CRDTSynchronizer {
    ...
    pub async fn start_gossip(&self) -> CRDTResult<()> {
        let mut interval = tokio::time::interval(
            tokio::time::Duration::from_millis(self.config.gossip_interval_ms)
        );
    ...
    async fn select_gossip_peers(&self) -> Vec<PeerInfo> {
        let peers = self.peers.read().await;
        let all_peers: Vec<_> = peers.values()
        ...
        let lagging_peers: Vec<_> = all_peers.iter()
            .filter(|p| our_clock.dominates(&p.last_known_clock))
            .cloned()
            .collect();
        
        let num_lagging = ((1.0 - self.config.random_gossip_probability) * self.config.fanout as f64) as usize;
    ...
    async fn create_gossip_message(&self) -> CRDTResult<GossipMessage> {
        ...
        let operations: Vec<_> = buffer.iter()
    ...
    async fn handle_gossip_response(&self, peer: &PeerInfo, response: &GossipResponse) -> CRDTResult<()> {
        ...
        for operation in &response.operations {
            self.operation_sender.send(operation.clone())
                .map_err(|_| CRDTError::NetworkError("Failed to send operation to handler".to_string()))?;
        }
    ...
    Self::Operation: Serialize + for<'a> Deserialize<'a>,
{
    ...
    fn serialize_operation(op: &Self::Operation) -> CRDTResult<Vec<u8>> {
        bincode::serialize(op)
            .map_err(|e| CRDTError::SerializationError(format!("Failed to serialize operation: {}", e)))
    }
    
    /// Deserialize an operation from gossip.
    fn deserialize_operation(data: &[u8]) -> CRDTResult<Self::Operation> {
        bincode::deserialize(data)
            .map_err(|e| CRDTError::SerializationError(format!("Failed to deserialize operation: {}", e)))
    }
}
...
mod tests {
    ...
    impl GossipTransport for MockTransport {
        async fn send_message(&self, _peer: &PeerInfo, message: GossipMessage) -> CRDTResult<GossipResponse> {
            {
