...
impl PNCounter {
    ...
    pub fn increment(&mut self, node_id: &NodeId, amount: u64) -> CRDTResult<()> {
        if amount == 0 {
            return Err(CRDTError::InvalidOperation(
                "Increment amount must be greater than 0".to_string(),
            ));
        }
    ...
    pub fn decrement(&mut self, node_id: &NodeId, amount: u64) -> CRDTResult<()> {
        if amount == 0 {
            return Err(CRDTError::InvalidOperation(
                "Decrement amount must be greater than 0".to_string(),
            ));
        }
    ...
    pub fn stats(&self) -> PNCounterStats {
        ...
        let (positive_nodes, negative_nodes, zero_nodes) =
            contributions
                .values()
        ...
        let min_contribution = contributions
            .values()
            .map(|(_, _, net)| *net)
            .min()
            .unwrap_or(0);
...
impl CRDT for PNCounter {
    ...
    fn apply_operation(&mut self, op: Self::Operation) -> Result<(), CRDTError> {
        match op {
            PNCounterOperation::Increment { node_id, amount } => self.increment(&node_id, amount),
            PNCounterOperation::Decrement { node_id, amount } => self.decrement(&node_id, amount),
        }
    }
