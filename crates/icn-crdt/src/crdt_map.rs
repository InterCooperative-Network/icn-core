...
use crate::{NodeId, VectorClock, CRDT, CRDTError, CRDTResult, CausalCRDT, CRDTValue, LWWRegister};
...
#[serde(bound = "K: CRDTValue + Eq + Hash + std::fmt::Debug + Serialize + for<'a> Deserialize<'a>, V: CRDT + Clone + Serialize + for<'a> Deserialize<'a>")]
    ...
    V: CRDT + Clone + Serialize + for<'a> Deserialize<'a>
{
    ...
    pub fn update_value(&mut self, key: &K, operation: V::Operation) -> CRDTResult<()> {
        if let Some(value) = self.get_mut(key) {
            ...
            Err(CRDTError::InvalidOperation(format!("Key not found: {:?}", key)))
        }
    }
    ...
    fn apply_remove(&mut self, key: K, timestamp: u64, node_id: NodeId) -> bool {
        ...
        let mut tombstone = self.tombstones.get(&key)
            .cloned()
            ...
            if tombstone.get().unwrap_or(&false) == &true {
                ...
                if other_clock.total_events() < tombstone_time {
                    operations.push(CRDTMapOperation::Remove {
                        ...
                        node_id: tombstone.get_writer_node().cloned()
    ...
    V::Operation: std::fmt::Debug + Serialize + for<'a> Deserialize<'a>
{
    ...
    fn apply_operation(&mut self, op: Self::Operation) -> Result<(), CRDTError> {
        match op {
            CRDTMapOperation::Put { key, value, timestamp, node_id } => {
                self.apply_put(key, value, timestamp, node_id)
            },
            CRDTMapOperation::Remove { key, timestamp, node_id } => {
                self.apply_remove(key, timestamp, node_id);
                Ok(())
            },
            CRDTMapOperation::UpdateValue { key, operation } => {
                self.update_value(&key, operation)
            }
        }
    }
...
mod tests {
    use super::*;
    use crate::{LWWRegister, GCounter};
    ...
    fn test_crdt_map_with_counters() {
        ...
        map.put("counter_a".to_string(), counter1, node_a()).unwrap();
        map.put("counter_b".to_string(), counter2, node_b()).unwrap();
