...
use crate::{CRDTError, CRDTResult, CRDTValue, CausalCRDT, NodeId, VectorClock, CRDT};
    ...
    T: CRDTValue,
{
    ...
    fn merge(&mut self, other: &Self) {
        ...
        if let Some(other_node) = &other.writer_node {
            if let Some(other_value) = &other.value {
                ...
                let _ = self.apply_clear(other.timestamp, other_node.clone(), other.sequence);
            }
        }
    ...
    fn apply_operation(&mut self, op: Self::Operation) -> Result<(), CRDTError> {
        match op {
            LWWRegisterOperation::Write {
                value,
                timestamp,
                node_id,
                sequence,
            } => self.apply_write(value, timestamp, node_id, sequence),
            LWWRegisterOperation::Clear {
                timestamp,
                node_id,
                sequence,
...
impl<T> Eq for LWWRegister<T> where T: CRDTValue + Eq + PartialEq {}
...
mod tests {
    ...
    fn test_lww_register_write() {
        ...
        register
            .write("second_value".to_string(), node_a())
            .unwrap();
    ...
    fn test_lww_register_clear() {
        let mut register = LWWRegister::with_initial_value("test".to_string(), 42u64, node_a());
    ...
    fn test_lww_register_concurrent_writes_tie_breaking() {
        ...
        register1
            .apply_write("value_a".to_string(), 5, node_a(), 1)
            .unwrap();
        register2
            .apply_write("value_b".to_string(), 5, node_b(), 1)
            .unwrap();
    ...
    fn test_lww_register_merge_idempotent() {
        let mut register1 =
            LWWRegister::with_initial_value("test".to_string(), "test_value".to_string(), node_a());
    ...
    fn test_lww_register_value_json() {
        let mut register =
            LWWRegister::with_initial_value("test".to_string(), "json_value".to_string(), node_a());
