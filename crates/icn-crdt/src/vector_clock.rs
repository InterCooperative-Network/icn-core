...
use crate::{NodeId, CRDTError, CRDTResult};
...
impl VectorClock {
    ...
    pub fn to_compact_bytes(&self) -> CRDTResult<Vec<u8>> {
        bincode::serialize(self)
            .map_err(|e| CRDTError::SerializationError(format!("Vector clock serialization failed: {}", e)))
    }
    
    /// Restore from compact representation.
    pub fn from_compact_bytes(bytes: &[u8]) -> CRDTResult<Self> {
        bincode::deserialize(bytes)
            .map_err(|e| CRDTError::SerializationError(format!("Vector clock deserialization failed: {}", e)))
    }
