...
mod libp2p_bootstrap_tests {
    ...
    async fn test_two_nodes_discover_each_other() {
        ...
        let node1_service = Libp2pNetworkService::new(config1).await.expect("Node 1 failed to start");
        ...
        let mock_listen_addr = "/ip4/127.0.0.1/tcp/0".parse::<Multiaddr>().expect("Invalid multiaddr");
        
        println!("Node 1 Peer ID: {}, using mock listen address: {}", node1_peer_id, mock_listen_addr);
        ...
        let _node2_service = Libp2pNetworkService::new(config2).await.expect("Node 2 failed to start");
