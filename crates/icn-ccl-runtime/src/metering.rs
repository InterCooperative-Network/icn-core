...
use crate::{current_timestamp, CclRuntimeError, WasmCode};
use icn_common::Did;
use icn_economics::{
    FileResourceLedger, ResourceLedger, ScopingRules, TokenClass, TokenClassId, TokenType,
    TransferabilityRule,
};
...
impl ManaMetering {
    ...
    pub fn with_config(config: MeteringConfig) -> Self {
        ...
        let _mana_token_class = TokenClass {
        ...
        Self {
            config,
            resource_ledger: Arc::new(RwLock::new(
                FileResourceLedger::new(PathBuf::from("mana_ledger.json")).unwrap(),
            )),
            mana_token_class_id,
    ...
    pub fn calculate_call_cost(&self, function: &str, args: &[u8], estimated_compute: u64) -> Mana {
    ...
    pub fn charge_mana(&self, did: &Did, amount: Mana) -> Result<(), CclRuntimeError> {
        let ledger = self.resource_ledger.write();
        ...
        ledger
            .burn(&self.mana_token_class_id, did, amount)
            .map_err(|e| {
                CclRuntimeError::ExecutionError(format!("Failed to charge mana: {}", e))
            })?;
    ...
    pub fn refund_mana(&self, did: &Did, amount: Mana) -> Result<(), CclRuntimeError> {
        ...
        if refund_amount > 0 {
            ...
            // Mint refund tokens
            ledger
                .mint(&self.mana_token_class_id, did, refund_amount)
                .map_err(|e| {
                    CclRuntimeError::ExecutionError(format!("Failed to refund mana: {}", e))
                })?;
    ...
    pub fn mint_initial_mana(&self, did: &Did, amount: Mana) -> Result<(), CclRuntimeError> {
        ...
        ledger
            .mint(&self.mana_token_class_id, did, amount)
            .map_err(|e| {
                CclRuntimeError::ExecutionError(format!("Failed to mint initial mana: {}", e))
            })?;
...
impl ManaMarket {
    ...
    pub fn convert_tokens(
        ...
        match (from_token, to_token) {
            ...
            _ => Err(CclRuntimeError::ExecutionError(format!(
...
mod tests {
    ...
    fn test_insufficient_mana() {
        ...
        match result.unwrap_err() {
            CclRuntimeError::InsufficientMana {
                required,
                available,
            } => {
