...
use icn_economics::monitoring::{
    AlertCategory, AlertSeverity, EconomicMonitoringService, MonitoringConfig, SystemHealthMetrics,
};
...
impl icn_economics::ManaLedger for TestManaLedger {
    fn get_balance(&self, _did: &icn_common::Did) -> u64 {
        0
    }
    fn set_balance(
        &self,
    ...
    fn spend(&self, _did: &icn_common::Did, _amount: u64) -> Result<(), icn_common::CommonError> {
        Ok(())
    }
    fn credit(&self, _did: &icn_common::Did, _amount: u64) -> Result<(), icn_common::CommonError> {
        Ok(())
    }
}
...
impl icn_economics::ResourceLedger for TestResourceLedger {
    ...
    fn transfer(
        &self,
        _class_id: &String,
        _from: &icn_common::Did,
        _to: &icn_common::Did,
        _amount: u64,
    ) -> Result<(), icn_common::CommonError> {
        Ok(())
    }
    fn get_balance(&self, _class_id: &String, _owner: &icn_common::Did) -> u64 {
        0
    }
    fn can_transfer(
        &self,
...
fn monitoring_service_creation() {
    ...
    let monitoring_service =
        EconomicMonitoringService::new(mana_ledger, resource_ledger, time_provider, config);
