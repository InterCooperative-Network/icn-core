...
use icn_economics::{
    ...
    InMemoryTrustProvider, ManaLedger, ManaSystemConfig, OrganizationType, RegenerativeManaLedger,
    SimpleEmergencyDetector, SimpleNetworkHealthProvider,
};
...
fn main() {
    ...
    let participants = setup_participants(&mut mana_ledger);

    println!("👥 Participants Setup:");
    for (name, did) in &participants {
        let account = mana_ledger.get_account_info(did).unwrap();
        println!(
            "   {}: {:?} (Trust: {:.1}, Participation: {:.1})",
            name, account.organization_type, account.trust_multiplier, account.participation_factor
        );
    }
    println!();
    ...
    demonstrate_emergency_modulation(&mut mana_ledger, &participants);
...
fn setup_participants(
    mana_ledger: &mut RegenerativeManaLedger<
        InMemoryLedger,
        DefaultHardwareMetricsProvider<FixedSystemInfoProvider>,
    ...
    mana_ledger
        .organization_provider()
        .set_federation_bonus(bob.clone(), 0.25);
    mana_ledger
        .trust_provider()
        .set_trust_multiplier(bob.clone(), 1.6);
    mana_ledger
        .trust_provider()
        .set_participation_factor(bob.clone(), 1.2);
    mana_ledger
        .trust_provider()
        .set_governance_engagement(bob.clone(), 1.5);
    ...
    mana_ledger
        .organization_provider()
        .set_organization_type(carol.clone(), OrganizationType::Community);
    mana_ledger
        .trust_provider()
        .set_trust_multiplier(carol.clone(), 1.2);
    mana_ledger
        .trust_provider()
        .set_participation_factor(carol.clone(), 1.1);
    mana_ledger
        .trust_provider()
        .set_governance_engagement(carol.clone(), 1.4); // High governance engagement
    ...
    mana_ledger
        .hardware_provider()
        .update_metrics(&carol, carol_hardware);
    ...
    mana_ledger
        .organization_provider()
        .set_organization_type(dave.clone(), OrganizationType::Unaffiliated);
    mana_ledger
        .trust_provider()
        .set_trust_multiplier(dave.clone(), 0.8);
    mana_ledger
        .trust_provider()
        .set_participation_factor(dave.clone(), 0.6);
    mana_ledger
        .trust_provider()
        .set_governance_engagement(dave.clone(), 0.7);
    participants.insert("Dave".to_string(), dave);

    participants
}

fn demonstrate_mana_operations(
    mana_ledger: &RegenerativeManaLedger<
        InMemoryLedger,
        DefaultHardwareMetricsProvider<FixedSystemInfoProvider>,
        InMemoryOrganizationProvider,
        InMemoryTrustProvider,
        ...
        SimpleNetworkHealthProvider,
    >,
    participants: &HashMap<String, Did>,
) {
    ...
    if let Err(e) = mana_ledger.credit(carol, 1000) {
        ...
        println!(
            "   ✅ Success! New balance: {}",
            mana_ledger.get_balance(carol)
        );
    }
    println!();
}

fn demonstrate_regeneration(
    mana_ledger: &RegenerativeManaLedger<
        InMemoryLedger,
        DefaultHardwareMetricsProvider<FixedSystemInfoProvider>,
        InMemoryOrganizationProvider,
...
fn demonstrate_emergency_modulation(
    mana_ledger: &mut RegenerativeManaLedger<
        InMemoryLedger,
        DefaultHardwareMetricsProvider<FixedSystemInfoProvider>,
        InMemoryOrganizationProvider,
    ...
    println!(
        "   Normal regeneration rate: {:.2} mana/hour",
        normal_rate * 100.0
    );
    ...
    println!(
        "   Emergency regeneration rate: {:.2} mana/hour",
        emergency_rate * 100.0
    );
    println!(
        "   Reduction factor: {:.1}%",
        (1.0 - emergency_rate / normal_rate) * 100.0
    );
...
fn demonstrate_compute_scores(
    ...
    for (name, did) in participants {
        ...
        println!(
            "     Success Rate: {:.1}%",
            hardware.job_success_rate * 100.0
        );
