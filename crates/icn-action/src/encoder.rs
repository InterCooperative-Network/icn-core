    ...
    pub fn encode(action: &Action) -> Result<String, ActionError> {
        ...
        match action {
            Action::ShareIdentity { did } => {
                url.set_path("share");
                url.query_pairs_mut().append_pair("did", &did.to_string());
            }
            ...
            Action::Vote {
                ...
                if let Some(voter) = voter {
                    url.query_pairs_mut()
                        .append_pair("voter", &voter.to_string());
                }
            }
            ...
            Action::SubmitJob {
                ...
                if let Some(cost) = max_cost {
                    url.query_pairs_mut()
                        .append_pair("max_cost", &cost.to_string());
                }
            }
        ...
        match path {
            ...
            "transfer" => {
                ...
                let amount = params
                    ...
                    .parse::<u64>()
                    .map_err(|_| {
                        ActionError::InvalidParameter("amount must be a number".to_string())
                    })?;
                ...
                let to =
                    Did::from_str(to_str).map_err(|_| ActionError::DidParsing(to_str.clone()))?;
            ...
            "submit" => {
                ...
                let max_cost = if let Some(cost_str) = params.get("max_cost") {
                    Some(cost_str.parse::<u64>().map_err(|_| {
                        ActionError::InvalidParameter("max_cost must be a number".to_string())
                    })?)
                } else {
                    None
                };
