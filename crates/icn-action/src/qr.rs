...
use crate::{Action, ActionError, ActionEncoder, QrMetadata, QrErrorCorrection, QrFormat};
...
impl QrGenerator {
    ...
    pub fn generate_png(_action_url: &str, _size: u32) -> Result<Vec<u8>, ActionError> {
        Err(ActionError::QrGeneration("QR feature not enabled. Compile with --features qr".to_string()))
    }
    
    /// Generate a QR code for an action with custom metadata
    #[cfg(feature = "qr")]
    pub fn generate_with_metadata(action_url: &str, metadata: &QrMetadata) -> Result<Vec<u8>, ActionError> {
    ...
    #[cfg(not(feature = "qr"))]
    pub fn generate_with_metadata(_action_url: &str, _metadata: &QrMetadata) -> Result<Vec<u8>, ActionError> {
        Err(ActionError::QrGeneration("QR feature not enabled. Compile with --features qr".to_string()))
    }
    
    /// Generate QR code for an action directly
    pub fn generate_for_action(action: &Action, metadata: &QrMetadata) -> Result<Vec<u8>, ActionError> {
    ...
    pub fn save_to_file(action_url: &str, file_path: &str, metadata: &QrMetadata) -> Result<(), ActionError> {
