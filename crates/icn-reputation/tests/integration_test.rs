...
fn test_trust_decay_models() {
    ...
    let exp_calculator = TrustDecayCalculator::with_time_decay(DecayModel::Exponential {
        half_life_seconds: 90 * 24 * 3600,
    });
    ...
    // Test linear decay
    let lin_calculator = TrustDecayCalculator::with_time_decay(DecayModel::Linear {
        decay_period_seconds: 90 * 24 * 3600,
    });
    ...
    let interaction_factor =
        exp_calculator.calculate_interaction_decay(&interactive_edge, old_time + 3600);
...
fn test_trust_aggregation() {
    ...
    let few_signals = vec![TrustSignal::DirectTrust {
        from: alice.clone(),
        to: bob.clone(),
        weight: 0.8,
    }];

    let low_confidence_result =
        aggregator.aggregate_trust_for_entity(&bob, few_signals, &time_provider);
...
fn test_aggregation_combination_methods() {
    ...
    let mean_result =
        mean_aggregator.aggregate_trust_for_entity(&bob, signals.clone(), &time_provider);
    ...
    let geom_result =
        geom_aggregator.aggregate_trust_for_entity(&bob, signals.clone(), &time_provider);
    ...
    let custom_config = AggregationConfig {
        combination_method: CombinationMethod::Custom {
            alpha: 2.0,
            beta: 1.0,
        },
        ..Default::default()
...
fn test_complex_trust_aggregation_scenario() {
    ...
    let aggregated_results =
        aggregator.aggregate_trust_for_entities(&all_entities, &graph, alice, &time_provider);
