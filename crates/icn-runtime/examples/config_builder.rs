...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Environment: {}", basic_config.environment.environment_type);
    ...
    println!("   Environment: {}", production_config.environment.environment_type);
    println!("   Listen addresses: {:?}", production_config.network.listen_addresses);
    println!("   DAG store: {}", production_config.storage.dag_store.store_type);
    println!("   Initial mana: {}", production_config.storage.mana_ledger.initial_mana);
    println!("   Governance enabled: {}\n", production_config.governance.enabled);
    ...
    println!("     Environment: {}", dev_config.environment.environment_type);
    println!("     Listen addresses: {:?}", dev_config.network.listen_addresses);
    println!("     mDNS enabled: {}", dev_config.network.enable_mdns);
    println!("     DAG store: {}", dev_config.storage.dag_store.store_type);
    ...
    println!("     Max incoming connections: {}", perf_config.network.connection_limits.max_incoming_connections);
    println!("     DAG cache size: {} MB", perf_config.storage.dag_store.cache_size_mb);
    ...
    println!("   Base environment: {}", env_config.environment.environment_type);
    println!("   Debug mode: {}", env_config.environment.debug);
    println!("   Log level: {}", env_config.environment.log_level);
    println!("   Custom DAG store: {}", env_config.storage.dag_store.store_type);
    println!("   Custom timeout: {} ms\n", env_config.network.timeouts.connection_timeout_ms);
    ...
    println!("   Merged environment: {}", merged_config.environment.environment_type);
    println!("   Merged DAG store: {}", merged_config.storage.dag_store.store_type);
    println!("   Merged governance: {}", merged_config.governance.enabled);
    println!("   Original node DID: {}\n", merged_config.identity.node_did);
    ...
    println!("   Bootstrap peers: {}", complex_config.network.bootstrap_peers.len());
    println!("   Data directory: {:?}", complex_config.storage.data_dir);
    println!("   Mana regeneration: {}/sec", complex_config.storage.mana_ledger.regeneration_rate);
    println!("   Voting period: {}s", complex_config.governance.voting.voting_period_seconds);
    println!("   Job timeout: {}ms\n", complex_config.runtime.job_execution_timeout_ms);
