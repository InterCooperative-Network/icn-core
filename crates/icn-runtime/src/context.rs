...
impl RuntimeContext {
                ...
                match self.did_resolver.resolve(&receipt.executor_did) {
                    Ok(vk) => {
                        if let Err(e) = receipt.verify_against_key(&vk) {
                            error!("[JobManagerDetail] Receipt signature verification failed for job {:?}: {}", job.id, e);
    ...
    pub async fn anchor_receipt(
        ...
        // Resolve the executor's verifying key and verify the signature
        let verifying_key = self
            .did_resolver
            .resolve(&receipt.executor_did)
            .map_err(|e| HostAbiError::SignatureError(format!("Failed to resolve DID: {}", e)))?;
