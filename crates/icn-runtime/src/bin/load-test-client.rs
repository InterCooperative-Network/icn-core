...
impl LoadTestRunner {
    ...
    async fn run_job_submission_test(
        ...
        info!("Starting job submission load test: rate={}/s, duration={:?}, job_size={}B, workers={}", 
            ...
            let handle = tokio::spawn(async move {
                ...
                while worker_start.elapsed() < duration {
                    ...
                    if total_requests.load(Ordering::Relaxed) % 100 == 0 {
                        debug!("Worker {}: {} requests completed", worker_id, 
        ...
        self.generate_results("job_submission", start_time, end_time, start_timestamp, end_timestamp).await
    }
    ...
    async fn run_burst_test(
        ...
        info!("Starting burst test: rate={}/s, duration={:?}", burst_rate, burst_duration);
        ...
        self.generate_results("burst", start_time, end_time, start_timestamp, end_timestamp).await
    }

    async fn run_governance_test(
        &self,
        rate: f64,
        duration: Duration,
    ) -> LoadTestResults {
        info!("Starting governance load test: rate={}/s, duration={:?}", rate, duration);
        ...
        self.generate_results("governance", start_time, end_time, start_timestamp, end_timestamp).await
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Test configuration: type={}, rate={}/s, duration={}s", 
    ...
    let results = match cli.test_type.as_str() {
        "job-submission" => {
            ...
            runner.run_job_submission_test(cli.rate, test_duration, job_size_bytes, cli.concurrent).await
        }
        "burst" => {
            let job_size_bytes = cli.job_size_mb * 1024 * 1024;
            runner.run_burst_test(cli.rate, test_duration, job_size_bytes).await
        }
        "large-jobs" => {
            let job_size_bytes = cli.job_size_mb * 1024 * 1024;
            runner.run_job_submission_test(cli.rate, test_duration, job_size_bytes, cli.concurrent).await
        ...
        "governance" => {
            runner.run_governance_test(cli.rate, test_duration).await
        }
        _ => {
    ...
    info!("Average response time: {:.2}ms", results.avg_response_time_ms);
    info!("95th percentile response time: {:.2}ms", results.p95_response_time_ms);
