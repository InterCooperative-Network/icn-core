...
use icn_dag::{
    conflict_resolution::{ConflictResolutionConfig, ConflictResolver, ResolutionStrategy},
...
fn test_federation_sync_complete_workflow() {
    ...
    // Verify the response
    if let Some(SyncMessage::SyncStatusResponse {
        from_node,
        block_count,
        ..
    }) = response
...
fn test_conflict_detection_and_resolution() {
    ...
    if let icn_dag::conflict_resolution::ResolutionStatus::Resolved { winner, .. } =
        resolution_status
    {
...
fn test_chain_fork_detection() {
    ...
    assert_eq!(
        ...
        icn_dag::conflict_resolution::ConflictType::ChainFork
    );
...
fn test_delta_sync_request() {
    ...
    let delta_request = sync_a
        .request_delta_sync(node_b_id.clone(), Some(block1.cid.clone()))
        .unwrap();

    // Verify the request
    if let SyncMessage::DeltaSyncRequest {
        ref from_node,
        ref since_root,
        max_blocks,
        ..
...
fn test_multi_criteria_conflict_resolution() {
    ...
    // Update CIDs
    let sig = None;
    early_block.cid = compute_merkle_cid(
        0x71,
        &early_block.data,
        &[],
        early_block.timestamp,
        &early_block.author_did,
        &sig,
        &None,
    );
    late_block.cid = compute_merkle_cid(
        0x71,
        &late_block.data,
        &[],
        late_block.timestamp,
        &late_block.author_did,
        &sig,
        &None,
    );
    ...
    let resolution_status = resolver
