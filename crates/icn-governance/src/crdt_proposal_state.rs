...
impl ProposalCRDT {
    ...
    pub fn new(metadata: ProposalMetadata, node_id: NodeId) -> Self {
        ...
        let votes_map = CRDTMap::new(format!("proposal_votes_{}", metadata_register.get().unwrap().proposal_id));
    ...
    pub fn get_metadata(&self) -> &ProposalMetadata {
        self.metadata.get().expect("Proposal metadata should always be present")
    }
    
    /// Update the proposal metadata.
    pub fn update_metadata(&mut self, new_metadata: ProposalMetadata, node_id: NodeId) -> Result<(), CommonError> {
        self.metadata.write(new_metadata, node_id)
    ...
    pub fn cast_vote(&mut self, vote: Vote, node_id: NodeId) -> Result<(), CommonError> {
        ...
        let mut vote_register = LWWRegister::with_initial_value(vote_id.clone(), vote.clone(), node_id.clone());
        
        self.votes.put(vote.voter.to_string(), vote_register, node_id)
...
impl CRDTProposalState {
    ...
    pub fn merge(&self, other: &Self) -> Result<(), CommonError> {
        let mut our_map = self.proposal_map.write()
            .map_err(|_| CommonError::LockError("Failed to acquire write lock".to_string()))?;
        
        let other_map = other.proposal_map.read()
    ...
    pub fn create_proposal(
        ...
        if self.config.max_proposals_per_proposer > 0 {
            ...
            if proposer_count >= self.config.max_proposals_per_proposer {
                return Err(CommonError::InvalidInputError(format!(
                    ...
                    proposer,
                    self.config.max_proposals_per_proposer
                )));
            }
        }
        ...
        let mut map = self.proposal_map.write()
            .map_err(|_| CommonError::LockError("Failed to acquire write lock".to_string()))?;
    ...
    pub fn cast_vote(
        ...
        debug!("Casting vote for proposal {} by {}: {:?}", proposal_id, voter, decision);
        ...
        debug!("Successfully cast vote for proposal {} by {}", proposal_id, voter);
        Ok(())
    }
    ...
    pub fn update_proposal_status(
        ...
        debug!("Updating proposal {} status to {:?}", proposal_id, new_status);
        ...
        for proposal_id in self.get_proposals_by_status(ProposalStatus::Open) {
            if let Ok(info) = self.get_proposal_info(&proposal_id) {
                if current_time > info.metadata.voting_deadline {
                    ...
                    let final_status = if info.has_quorum && info.vote_tally.approval_percentage >= info.metadata.required_approval {
    ...
    fn get_proposal_crdt(&self, proposal_id: &str) -> Result<ProposalCRDT, CommonError> {
        let map = self.proposal_map.read()
            .map_err(|_| CommonError::LockError("Failed to acquire read lock".to_string()))?;
    ...
    fn update_proposal_crdt(&self, proposal_id: &str, proposal_crdt: ProposalCRDT) -> Result<(), CommonError> {
        let mut map = self.proposal_map.write()
            .map_err(|_| CommonError::LockError("Failed to acquire write lock".to_string()))?;
...
mod tests {
    ...
    fn test_proposal_quorum_and_approval() {
        ...
        // First vote
        state.cast_vote("proposal_1", bob_did(), VoteDecision::Approve, None).unwrap();
        ...
        state.cast_vote("proposal_1", charlie_did(), VoteDecision::Approve, None).unwrap();
    ...
    fn test_get_proposals_by_status() {
        ...
        state.update_proposal_status("proposal_2", ProposalStatus::Approved).unwrap();
    ...
    fn test_get_proposals_by_proposer() {
        ...
        state.create_proposal("proposal_3".to_string(), "P3".to_string(), "D3".to_string(), bob_did(), None, None, None).unwrap();
    ...
    fn test_merge_proposal_states() {
        ...
        state2.create_proposal("proposal_2".to_string(), "P2".to_string(), "D2".to_string(), bob_did(), None, None, None).unwrap();
        
        // Cast votes on different nodes for the same proposal (if it existed on both)
        state1.create_proposal("shared_proposal".to_string(), "Shared".to_string(), "Shared proposal".to_string(), charlie_did(), Some(current_timestamp() + 3600), None, None).unwrap();
        state2.create_proposal("shared_proposal".to_string(), "Shared".to_string(), "Shared proposal".to_string(), charlie_did(), Some(current_timestamp() + 3600), None, None).unwrap();
        
        state1.cast_vote("shared_proposal", alice_did(), VoteDecision::Approve, None).unwrap();
        state2.cast_vote("shared_proposal", bob_did(), VoteDecision::Reject, None).unwrap();
    ...
    fn test_proposal_stats() {
        ...
        state.cast_vote("proposal_1", alice_did(), VoteDecision::Approve, None).unwrap();
        state.cast_vote("proposal_1", bob_did(), VoteDecision::Reject, None).unwrap();
        state.cast_vote("proposal_2", charlie_did(), VoteDecision::Approve, None).unwrap();
    ...
    fn test_max_proposals_per_proposer() {
        ...
        let result = state.create_proposal("proposal_3".to_string(), "P3".to_string(), "D3".to_string(), alice_did(), None, None, None);
        assert!(result.is_err());
        
        // But Bob should still be able to create proposals
        let result = state.create_proposal("proposal_4".to_string(), "P4".to_string(), "D4".to_string(), bob_did(), None, None, None);
        assert!(result.is_ok());
    }
}