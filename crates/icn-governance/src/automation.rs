...
impl GovernanceAutomationEngine {
    ...
    pub async fn process_proposal_submission(
        ...
        // Store state
        self.active_proposals.write().unwrap().insert(proposal_id.clone(), automation_state);
        self.voting_participants.write().unwrap().insert(proposal_id.clone(), eligible_voters);
    ...
    pub async fn process_vote_cast(
        ...
        if let Some(mut state) = self.active_proposals.write().unwrap().get_mut(&proposal_id) {
            state.votes_received.insert(voter.clone(), (vote.clone(), vote_weight.clone()));
            ...
            if !state.voting_status.quorum_reached && self.check_quorum(&state).await? {
                ...
                if state.voting_status.support_percentage >= self.config.auto_execution_threshold {
                    self.attempt_automatic_execution(&proposal_id, &voting_result).await?;
                }
            }
        }
    ...
    async fn start_policy_enforcement_loop(&self) -> Result<tokio::task::JoinHandle<()>, CommonError> {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = Self::enforce_policies(
                    &policy_cache,
                    &ccl_runtime,
                    &mana_ledger,
                    &reputation_store,
    ...
    async fn start_predictive_analysis_loop(&self) -> Result<tokio::task::JoinHandle<()>, CommonError> {
        let active_proposals = self.active_proposals.clone();
        let reputation_store = self.reputation_store.clone();
                    ...
                    if state.voting_status.support_percentage >= config.auto_execution_threshold {
                        ...
                        tokio::spawn(async move {
                            if let Err(e) = Self::execute_proposal_async(
                                ...
                                log::error!("Failed to execute proposal {:?}: {}", proposal_id_clone, e);
                            }
                        });
                    }
                }
                ...
                for &reminder_interval in &config.voting_reminder_intervals {
                    if time_elapsed >= reminder_interval {
                        let reminder_type = match reminder_interval {
                            ...
                            d if d >= Duration::from_secs(3 * 3600) => ReminderType::DeadlineApproaching,
    ...
    async fn determine_eligible_voters(&self, _proposal: &Proposal) -> Result<Vec<Did>, CommonError> {
    ...
    async fn calculate_voting_status(&self, state: &ProposalAutomationState) -> Result<VotingStatus, CommonError> {
    ...
    pub fn get_automation_stats(&self) -> GovernanceAutomationStats {
        ...
        let awaiting_votes = proposals.values().filter(|s| s.votes_received.len() < s.eligible_voters.len()).count();
        let quorum_reached = proposals.values().filter(|s| s.voting_status.quorum_reached).count();
        let auto_executable = proposals.values()
...
mod tests {
    ...
    fn test_vote_weight_calculation() {
        ...
        assert_eq!(weight.total_weight, weight.base_weight * weight.reputation_multiplier);
    }
