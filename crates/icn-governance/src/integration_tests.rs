...
mod tests {
    ...
    fn test_complete_governance_workflow() {
        ...
        // Test voting
        governance.cast_vote(voter_did.clone(), &proposal_id, VoteOption::Yes).unwrap();
        
        // Test closing voting period
        let (status, (yes_votes, no_votes, abstain_votes)) = governance.close_voting_period(&proposal_id).unwrap();
    ...
    fn test_vote_delegation() {
        ...
        // Set up delegation
        governance.delegate_vote(delegator.clone(), delegate.clone()).unwrap();
        ...
        governance.cast_vote(delegate.clone(), &proposal_id, VoteOption::Yes).unwrap();
    ...
    fn test_event_store_integration() {
        ...
        let proposal_id = governance.submit_proposal(submission).unwrap();
        governance.open_voting(&proposal_id).unwrap();
        governance.cast_vote(voter_did, &proposal_id, VoteOption::Yes).unwrap();
        ...
        if let Some(store) = governance.event_store() {
            ...
            let has_proposal_submitted = events.iter().any(|e| {
                matches!(e, GovernanceEvent::ProposalSubmitted(_))
            });
            let has_vote_cast = events.iter().any(|e| {
                matches!(e, GovernanceEvent::VoteCast(_))
            });
    ...
    fn test_member_management() {
        ...
        let proposal_id = governance.submit_proposal(submission).unwrap();
        governance.open_voting(&proposal_id).unwrap();
        governance.cast_vote(member2, &proposal_id, VoteOption::Yes).unwrap();
