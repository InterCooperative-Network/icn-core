...
mod p2p_dag_e2e_tests {
    ...
    use icn_protocol::{
        DagBlockAnnouncementMessage, DagBlockRequestMessage, ExecutionMetadata,
        MeshReceiptSubmissionMessage, MessagePayload, ProtocolMessage,
    };
    ...
    impl TestNode {
        ...
        fn create_test_block(&self, data: &[u8], links: Vec<Cid>) -> DagBlock {
            let dag_links: Vec<DagLink> = links
                .into_iter()
                .map(|c| DagLink {
                    cid: c,
        ...
        async fn get_connectivity_stats(
            &self,
        ) -> Result<icn_network::NetworkStats, Box<dyn std::error::Error + Send + Sync>> {
    ...
    impl TestNetwork {
        ...
        async fn new(node_count: usize) -> Self {
            ...
            assert!(
                ...
                "Bootstrap node must have listening addresses"
            );
        ...
        async fn verify_connectivity(&self) {
            ...
            for (i, node) in self.nodes.iter().enumerate() {
                ...
                info!(
                    "Node {}: {} peers, {} messages sent, {} messages received",
        ...
        async fn test_distributed_storage(
            ...
            assert!(
                ...
                "Node 1 should be able to retrieve the block"
            );
            assert_eq!(
        ...
        async fn test_load_resilience(
            ...
            info!(
                "Testing network resilience with {} operations",
                num_operations
            );
            ...
            info!(
                "Load test completed: {}/{} operations succeeded in {:?} ({:.2} ops/sec)",
    ...
    async fn test_network_message_propagation() {
        ...
        for (i, node) in network.nodes.iter().enumerate() {
            ...
            info!(
                "Node {} sent {} messages, received {} messages",
